...
\begin{frame}{Income and Wealth Shares Around the World}
  \small
  In every world region, the top 10\% earns a far larger share of income than the bottom 50\% --- and the gap is wider still for wealth, where the top 10\% typically owns 60--80\% and the bottom half less than 5\%.
  \vspace{0.3em}
  \begin{center}
  \begin{tabular}{@{}lccc@{}}
    \toprule
    \textbf{Region (2021)} & \textbf{Bottom 50\%} & \textbf{Middle 40\%} & \textbf{Top 10\%} \\
    \midrule
    Europe & 19\% & 45\% & 36\% \\
    East Asia & 14\% & 42\% & 43\% \\
  ...
  \end{center}
  \centering
  Europe is the world's most equal region by this measure --- but still far from equal.\\
    ...
    \begin{column}{0.55\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
          ...
          \addplot[fill=wulight, fill opacity=0.4, draw=none] coordinates {(0,0) (50,20) (90,65) (100,100)} \closedcycle;
          \addplot[wugray, dashed, thick, domain=0:100] {x};
          \node[wugray, font=\scriptsize] at (axis cs:88,96) {45\textdegree};
          \addplot[wublue, thick, smooth, mark=none] coordinates {(0,0) (50,20) (90,65) (100,100)};
          \node[wublue, font=\small] at (axis cs:55,42) {$A$};
          \addplot[wugray, dotted] coordinates {(50,0) (50,20)};
          \addplot[wugray, dotted] coordinates {(0,20) (50,20)};
          \addplot[wugray, dotted] coordinates {(90,0) (90,65)};
          \addplot[wugray, dotted] coordinates {(0,65) (90,65)};
        \end{axis}
      \end{tikzpicture}
      \end{center}
    \end{column}
...
\begin{frame}{The Rise of Private Wealth, the Fall of Public Wealth}
  \small
  \begin{itemize}
    \item Across rich countries since 1970, \textbf{private} wealth has risen sharply relative to national income --- while \textbf{public} wealth has stagnated or turned negative.
    \item Austria's own version of the public-wealth story: the abolition of the general \textbf{wealth tax} (\emph{Vermögensteuer}) in 1994, and the lapse of the \textbf{inheritance/gift tax} in 2008 (Lecture 4), removed two of the main tools governments elsewhere still use to tax accumulated private wealth.
  \end{itemize}
  \vspace{0.3em}
  \centering
  A government that owns less (or owes more) has less room to cushion shocks, or to redistribute, without new borrowing or taxation.\\
  \srcnote{Blanchet \& Martínez-Toledano, ``Wealth Inequality Dynamics in Europe and the United States,'' \emph{Journal of Monetary Economics} 133, 2023.}
\end{frame}
...
\begin{frame}{The US Is Less Mobile than Denmark}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      xmin=0, xmax=100, ymin=25, ymax=70,
      xlabel={Parent income rank}, ylabel={Mean child income rank},
      axis lines=left,
      ticks=none,
      ]
      \addplot[wublue, thick, domain=0:100] {41+0.18*x};
      \node[wublue, font=\scriptsize, anchor=south west] at (axis cs:4,43) {Denmark (0.180)};
      \addplot[wugray, thick, domain=0:100] {30+0.34*x};
      \node[wugray, font=\scriptsize, anchor=north west] at (axis cs:4,31.5) {United States (0.341)};
    \end{axis}
  \end{tikzpicture}
  \end{center}
  \centering
...
\begin{frame}{Mobility Varies Enormously \emph{Within} Countries Too}
  \small
  \begin{itemize}
    \item Chetty et al.\ (2014) map absolute upward mobility across every US commuting zone using anonymized tax records --- the interactive \textbf{Opportunity Atlas} lets users explore outcomes down to the neighborhood level.
    \item Mobility is systematically \textbf{lower} in the US South, and higher in the Midwest, Great Plains, and much of the Northeast --- differences as large as the US-vs-Denmark gap on the previous slide, found \emph{within a single country}.
  \end{itemize}
  \vspace{0.3em}
  \centering
  \alert{The lesson generalizes: national averages can hide as much as they reveal --- the same is likely true of mobility across Austria's own regions, even without an Opportunity-Atlas-style tool to show it directly.}\\
