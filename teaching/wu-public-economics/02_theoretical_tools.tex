...
\begin{frame}{Austria's Sozialhilfe (Minimum Income Support)}
  \begin{itemize}
    ...
    \item A 2019 reform introduced the \emph{Wiedereinsteigerfreibetrag}: up to 35\% of net earnings are disregarded (not counted against the benefit) for up to 12 months when a recipient starts working.
  \end{itemize}
  \vspace{0.3em}
  \centering
  This is Austria's own version of Gruber's TANF example --- a benefit guarantee plus a benefit-reduction rate.
  \srcnote{ÖGB, ``So viel Sozialhilfe bekommt man in Österreich''; Sozialministerium, Sozialhilfe-Grundsatzgesetz.}
\end{frame}
...
\begin{frame}{Sources (2/2): Austrian Data \& Institutions}
  ...
  \begin{itemize}
  \setlength{\itemsep}{2pt}
    \item Sozialministerium: Sozialhilfe-Grundsatzgesetz; ``Armutsgefährdung und soziale Ausgrenzung von Ein-Eltern-Haushalten''
