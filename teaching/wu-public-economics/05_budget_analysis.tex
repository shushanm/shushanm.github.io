...
\begin{frame}{Budgets, Deficits, and Debt: The Basics}
  \begin{block}{Three related concepts}
    \textbf{Budget} = planned spending and revenue for a fiscal year.\\
    \textbf{Deficit} = spending $>$ revenue in a given year (a \emph{flow}).\\
    \textbf{Debt} = accumulated past deficits (a \emph{stock}).
  \end{block}
  \vspace{0.3em}
  \begin{itemize}
    \item The EU's \textbf{Maastricht rules} cap the deficit at 3\% of GDP and gross debt at 60\% of GDP for member states (Lecture 1).
...
\begin{frame}{Austria's Budget Cycle}
  ...
  \end{enumerate}
  \srcnote{Parlament Österreich, ``Wie berät der Nationalrat das Budget?''; BMF.}
\end{frame}

\begin{frame}{Planning Four Years Ahead: The BFRG}
  \small
  \begin{itemize}
    \item Since 2009, Austria's medium-term framework (\textbf{BFRG}) sets binding expenditure ceilings for a \textbf{rolling four-year horizon} --- each year, a new fourth year is added.
    \item Since 2017, the BFRG is debated and decided \emph{together} with the annual BFG each autumn.
    \item The 2009 reform also enabled genuine \textbf{two-year budgets} (\emph{Doppelbudget}): used for 2013/14, 2018/19, and most recently \textbf{2025/26} (passed June 2025, alongside a BFRG running to 2029).
  \end{itemize}
  \vspace{0.3em}
  \centering
  A rolling four-year window is Austria's answer to a problem every government budget faces: today's decisions have consequences years beyond the current fiscal year.\\
  \srcnote{Parlament Österreich; BMF, Doppelbudget 2025/2026 press release (June 2025).}
\end{frame}
...
\begin{frame}{Der Fiskalrat: Austria's Fiscal Watchdog}
  ...
  \begin{itemize}
    ...
    \item Institutionally housed at the \textbf{Oesterreichische Nationalbank (OeNB)} since 1997; a small dedicated office (around 6 staff) supports it.
    \item It is Austria's designated \textbf{Independent Fiscal Institution} under EU Regulation 473/2013: it monitors compliance with national and EU fiscal rules and publishes an annual macro-fiscal outlook.
  \end{itemize}
  \vspace{0.3em}
  \begin{alertblock}{Not quite the US Congressional Budget Office}
    The CBO (est.\ 1974, $\sim$275 staff) formally \emph{scores} pending legislation for Congress. The much smaller Fiskalrat does not score individual bills --- it monitors and reports, closer to a fiscal watchdog than a legislative scorekeeper.
  \end{alertblock}
  \srcnote{Fiskalrat.at, ``Organisation''; CBO, ``10 Things to Know About CBO.''}
\end{frame}
...
\begin{frame}{Keeping Bund, Länder, and Gemeinden in Line}
  ...
  \end{itemize}
  \vspace{0.3em}
  \begin{alertblock}{Does Austria have a constitutional ``debt brake''?}
    No. Unlike Germany's constitutionally entrenched \emph{Schuldenbremse}, attempts to anchor one in Austria's federal constitution failed to reach the required two-thirds majority --- first in \textbf{December 2011}, then again in the Bundesrat in \textbf{October 2019}. Austria's debt brake exists only as \emph{ordinary law} (Art.\ 4 ÖStP 2012; §2(4) BHG 2013).
  \end{alertblock}
  \srcnote{Parlament Österreich, debate archive 7.12.2011; RIS, Österreichischer Stabilitätspakt 2012.}
\end{frame}

\begin{frame}{The EU Layer: Maastricht Rules, Reformed}
  \footnotesize
  \begin{itemize}
    \item The original Maastricht reference values --- \textbf{3\% deficit-to-GDP, 60\% debt-to-GDP} --- are unchanged since the Treaty. What changed, in a reform that took effect \textbf{30 April 2024}, is how compliance is planned and enforced.
    \item The \textbf{net expenditure path} is now the single operational indicator: each member state agrees a multi-year fiscal-structural plan with the Council, tracked via a ``control account.''
    \item The standard adjustment period is \textbf{4 years}, extendable to \textbf{7 years} if the country commits to specified reforms and investments.
  \end{itemize}
  \vspace{0.3em}
  \begin{alertblock}{Austria, right now}
    The EU Council opened an \textbf{excessive deficit procedure} against Austria on 8 July 2025, triggered by a 2024 deficit of 4.7\% of GDP --- with an approved path requiring the deficit below 3\% by 2028.
  \end{alertblock}
  \srcnote{Consilium, ``Council adopts reform of fiscal rules'' (29 Apr 2024); European Parliament fact sheet.}
\end{frame}
...
\begin{frame}{Application: What Happens When a Fiscal Rule Is Relaxed?}
  ...
  \vspace{0.3em}
  \centering
  Fiscal rules like Austria's ÖStP or the EU's expenditure path are not just accounting devices --- they bind real political incentives to spend and tax.\\
...
\begin{frame}{Real vs.\ Nominal: The Inflation Tax}
  ...
  \srcnote{Statistik Austria, Öffentliche Finanzen 2025 (debt revision); Statistik Austria, VPI Jahr 2024.}
\end{frame}
...
\begin{frame}{Cyclical vs.\ Structural Deficits}
  ...
  \end{itemize}
  \vspace{0.3em}
  \begin{alertblock}{Who computes this for Austria?}
    Both the \textbf{Fiskalrat} and the \textbf{European Commission} publish structural-balance estimates for Austria --- the Commission's version is the actual basis for EU fiscal-rule compliance, not the headline (nominal) deficit.
  \end{alertblock}
  \srcnote{Fiskalrat, Jahresbericht; European Commission, DG ECFIN.}
\end{frame}
...
\begin{frame}{Cash vs.\ Accrual Accounting: The International Experience}
  ...
  \srcnote{Gruber, \emph{Public Finance and Public Policy}, Ch.\ 4, \S4.2.}
\end{frame}
...
\begin{frame}{Austria's Own Shift: The Haushaltsrechtsreform}
  ...
  \end{tabular}
  \end{center}
  \srcnote{BMF, ``Die Haushaltsrechtsreform des Bundes''; BHAG, Geschichte.}
\end{frame}
...
\begin{frame}{Static vs.\ Dynamic Scoring}
  ...
  \vspace{0.3em}
  \centering
  In Austria, both Parliament's \textbf{Budgetdienst} and independent institutes (\textbf{WIFO}, \textbf{IHS}) routinely cost major policy proposals --- the same static-vs-dynamic tension applies to their estimates.\\
  \srcnote{Gruber, \emph{Public Finance and Public Policy}, Ch.\ 4, \S4.2.}
\end{frame}
...
\begin{frame}{Worked Example: Should the Government Pave the Road?}
  ...
  \end{center}
  \vspace{0.3em}
  \begin{alertblock}{The politician's mistake}
    ``It pays for itself in exactly 10 years, so it's obviously worth it'' ignores that the later savings are worth \emph{less in today's terms} than the euro spent now --- at any positive interest rate, this project is a bad investment.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Why Current Debt Labels Can Mislead}
  ...
  \srcnote{Gruber, \emph{Public Finance and Public Policy}, Ch.\ 4, \S4.3.}
\end{frame}
...
\begin{frame}{Measuring Long-Run Sustainability: Austria's Pension System}
  ...
  \end{itemize}
  \srcnote{European Commission, 2024 Ageing Report --- Austria Country Fiche; Sozialministerium, Alterssicherungskommission.}
\end{frame}

\begin{frame}{A Much Flatter Curve Than the US Story}
  \small
  Gruber's textbook centers this section on the US Social Security and Medicare trustees' long-run \textbf{fiscal imbalance} --- a headline figure in the tens of trillions of dollars, driven by healthcare-cost growth as much as by aging itself.
  \vspace{0.3em}
  \begin{alertblock}{Austria's trajectory looks very different}
    A $\sim$0.3-point rise in pension spending as a share of GDP over 48 years is a far flatter path than the US long-run imbalance. Two reasons: Austria's pension system was already reformed repeatedly (raising the effective retirement age, harmonizing civil-servant pensions); and --- unlike the US case --- this projection doesn't have to absorb open-ended healthcare-cost growth on the same scale.
  \end{alertblock}
  \centering
  The lesson generalizes: \emph{how} a long-run projection is measured, and \emph{which} obligations it includes, drives the headline number as much as demographics alone.
\end{frame}
...
\begin{frame}{How Far Ahead Should We Look? Budget Windows}
  ...
  \begin{itemize}
    ...
    \item Austria's own answer sits in between on the \emph{annual} planning side: the BFRG's rolling \textbf{4-year} window (\S5.1) --- extendable to \textbf{7 years} under the EU's reformed fiscal framework if reforms and investments are committed to.
  \end{itemize}
  \vspace{0.3em}
  \centering
  There is no ``correct'' window --- only a trade-off between capturing genuinely long-run obligations and trusting projections far enough out that the numbers stop being credible.\\
  \srcnote{Gruber, \emph{Public Finance and Public Policy}, Ch.\ 4, \S4.3; European Parliament, EU fiscal framework fact sheet.}
\end{frame}
...
\begin{frame}{How a Deficit Can Crowd Out Private Investment}
  \begin{columns}[T]
    ...
    \end{column}
  \end{columns}
  \srcnote{Gruber, \emph{Public Finance and Public Policy}, Ch.\ 4, \S4.4 (Fig.\ 4-4).}
\end{frame}
...
\begin{frame}{Open Capital Markets Blunt Crowding Out}
  ...
  \end{itemize}
  \vspace{0.3em}
  \begin{alertblock}{This fits a small open economy like Austria especially well}
    As one country inside the euro area's single capital market, Austria effectively faces a supply of savings that is close to perfectly elastic --- far more so than a large economy like the US, where domestic savings still matter enough that some crowding-out concern remains empirically credible.
  \end{alertblock}
  \centering
  The theoretical case for worrying about crowding-out is \emph{weaker}, not stronger, for a small Eurozone member than for the textbook's own (US) example.\\
  \srcnote{Gruber, \emph{Public Finance and Public Policy}, Ch.\ 4, \S4.4.}
\end{frame}

\begin{frame}{The Secular Stagnation Puzzle}
  \small
  Real interest rates across the industrialized world have fallen sharply and persistently over the past four decades --- and not just in one country:
  \vspace{0.3em}
  \begin{center}
  \begin{tabular}{@{}lc@{}}
    \toprule
  ...
  \srcnote{Furman \& Summers (2020), cited in Gruber, \emph{Public Finance and Public Policy}, Ch.\ 4, Fig.\ 4-5.}
\end{frame}
...
\begin{frame}{Austria and the Negative-Rate Era}
  ...
  \vspace{0.3em}
  \centering
  This is the Eurozone's own lived version of the secular-stagnation puzzle --- and the backdrop against which Austria's rising post-2020 debt ratio has to be judged.\\
  \srcnote{European Central Bank, key interest rate history.}
\end{frame}
...
\begin{frame}{The ``New View'' of Deficits}
  ...
  \end{itemize}
  \vspace{0.3em}
  \begin{alertblock}{Why this matters for Austria's debate}
    Austria's Fiskalrat frames its warnings in the traditional debt-ratio terms (\S5.1) --- while the ``new view'' would ask instead how much of the deficit funds productive investment versus current consumption.
  \end{alertblock}
  \srcnote{Furman \& Summers (2020), ``Fiscal Sustainability of the United States''; Gruber, \emph{Public Finance and Public Policy}, Ch.\ 4, \S4.4.}
\end{frame}
...
\begin{frame}{Highlights}
  \small
  \begin{itemize}
    \item A government budget is shaped as much by \emph{process} --- Austria's BFRG, Fiskalrat, ÖStP, and the EU's fiscal framework --- as by economics alone.
    \item ``The deficit'' is not one number: nominal vs.\ real, cyclical vs.\ structural, and cash vs.\ accrual measures can all tell different stories about the same year.
    \item Present discounted value is the right tool for judging any policy that trades an upfront cost for a future stream --- and it is exactly what long-run fiscal-sustainability measures (like pension projections) try to capture.
    \item The case for deficits crowding out investment is theoretically weaker for a small, financially open economy like Austria than for a large one --- and persistently low real interest rates since the 1980s have made economists rethink how much to worry about debt levels at all.
  \end{itemize}
\end{frame}
...
\begin{frame}{Discussion Questions for Next Time}
  \small
  \begin{enumerate}
    \item Austria's debt brake exists only as ordinary law, not constitutional law. Does that make it a meaningfully weaker commitment device --- and if so, for whom?
    \item The EU's reformed fiscal framework lets Austria take up to seven years to adjust, if it commits to reforms and investment. What kinds of ``reforms and investment'' should qualify, and who should judge that?
    \item Grembi, Nannicini \& Troiano found relaxed fiscal rules led Italian mayors to cut taxes and raise deficits, especially where re-election was possible. What does this suggest about designing fiscal rules for Austria's Länder and Gemeinden?
    \item If persistently low real interest rates really do make the ``new view'' of deficits correct, should Austria be borrowing \emph{more} to invest --- and in what?
    \item Austria's pension projections rise far less steeply than the US Social Security/Medicare story. Is that because Austria's demographic pressure is genuinely smaller, or because of how the obligation is defined and measured?
  \end{enumerate}
\end{frame}
...
\begin{frame}{Sources}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
      \setlength{\itemsep}{3pt}
        \item Textbook: Gruber, J. \emph{Public Finance and Public Policy}, Ch.\ 4 (\S4.1--4.5, Budget Analysis and Deficit Financing), Worth Publishers.
        \item Grembi, V., Nannicini, T., \& Troiano, U., ``Do Fiscal Rules Matter?'' \emph{American Economic Journal: Applied Economics} 8(3), 2016.
        \item Furman, J., \& Summers, L., ``A Reconsideration of Fiscal Policy in the Era of Low Interest Rates'' (2020).
        \item Parlament Österreich: Häufige Fragen zum Budget; Wie berät der Nationalrat das Budget?; Doppelbudget 2025/2026.
        \item Bundesministerium für Finanzen (BMF): Doppelbudget 2025/2026 (Presseaussendung); Die Haushaltsrechtsreform des Bundes.
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
      \setlength{\itemsep}{3pt}
        \item Fiskalrat.at, Organisation und Jahresbericht 2025.
        \item RIS: Österreichischer Stabilitätspakt 2012; Bundeshaushaltsgesetz 2013.
        \item Consilium/European Parliament: reform of the EU economic governance framework (2024).
        \item Statistik Austria: Öffentliche Finanzen 2025; Verbraucherpreisindex Jahr 2024.
        \item European Commission, 2024 Ageing Report --- Austria Country Fiche; Sozialministerium, Alterssicherungskommission.
        \item European Central Bank, key interest rate history.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
